...
func working() {
	testBrackets := "{z :=((-1))}"
	testAddition := "{z :=2+(-1)}"
	assignNegativeValueWithGrouping := "{z :=(-1)}"
	...
	simplePlus := "{x :=0; x = x+1}"
	doubleGrouping := "{z :=((-1))}"
	groupingWithOperationAfterwards := "{z :=(-1)+2}"
	...
	testIfElse2 := "{x := 0;y:=0; if 4 < x {x = x+1; x = x*10; y:= 100; y = y * 2} else {y=1}}"
	...
	runstmt(parse(assignNegativeValueWithGrouping))
	...
	runstmt(parse(simplePlus))
	runstmt(parse(doubleGrouping))
	runstmt(parse(groupingWithOperationAfterwards))
	runstmt(parse(testAndLesser))
	runstmt(parse(testLesserAnd))
	runstmt(parse(testIfElse))
	runstmt(parse(testIfElseElse))
	runstmt(parse(testIfElse2))
